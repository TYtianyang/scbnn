# -*- coding: utf-8 -*-
"""
Created on Tue Nov 10 10:43:41 2020
...
## define SCBNN: the multi layers Bayes network with shape constraints
class SCBNN(nn.Module):
    def __init__(self, data, test,
                 neuron = 40, alpha = 5, 
                 bayes_lr = 1e-2, bayes_step = 200, bayes_gap = 20):
        super().__init__()
        ...
        self.negative_log_likelihood_path = []
        self.loss_c4_path = []
        self.loss_c5_path = []
        self.c4_path = []
        self.c5_path = []
    ...
    # define skew_normal: the discount factor by approximation to skewed normal pdf
    def skew_normal(self, x):
        piece2 = (2/np.sqrt(2*np.pi))*torch.exp(-(torch.square(x))/2)*(np.pi + self.alpha*x + torch.sin(self.alpha*x))/(2*np.pi)
        piece3 = (2/np.sqrt(2*np.pi))*torch.exp(-(torch.square(x))/2)*(1- (np.pi + self.alpha*x + torch.sin(self.alpha*x))/(2*np.pi))
        return piece2*(x>(-np.pi/self.alpha)).float() + piece3*(x>(np.pi/self.alpha)).float()
    ...
    def loss_c4(self):
        ...
        c4_loss = - torch.log(self.skew_normal(p_t).mean())
        return(c4_loss, p_t.mean())
    ...
    def loss_c5(self):
        ...
        c5_loss = - torch.log(self.skew_normal(c5).mean())
        return(c5_loss, c5.mean())
    ...
    def elbo(self):
        ...
        loss_c4, c4 = self.loss_c4()
        loss_c5, c5 = self.loss_c5()

        loss = log_variational_posterior - log_prior + negative_log_likelihood + loss_c4 + loss_c5
        return loss, log_prior, log_variational_posterior, negative_log_likelihood, loss_c4, loss_c5, c4, c5

    # define bayes_fit: the fitting function
    def bayes_fit(self):
        if self.sample_mode=="mean":
            optimizer = torch.optim.Adam([self.l1.weight_mu,self.l2.weight_mu,self.l3.weight_mu,self.l4.weight_mu,
                                          self.l1.bias_mu,self.l2.bias_mu,self.l3.bias_mu,self.l4.bias_mu], lr=self.bayes_lr)
        else:
            optimizer = torch.optim.Adam([self.l1.weight_rho,self.l2.weight_rho,self.l3.weight_rho,self.l4.weight_rho,
                                          self.l1.bias_rho,self.l2.bias_rho,self.l3.bias_rho,self.l4.bias_rho], lr=self.bayes_lr)
        tqdm_list = tqdm(range(self.bayes_step))
        index = 1
        loss0 = np.inf
        for t in tqdm_list:
            loss, log_prior, log_variational_posterior, negative_log_likelihood, loss_c4, loss_c5, c4, c5 = self.elbo()
            self.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            self.negative_log_likelihood_path.append(negative_log_likelihood.item())
            self.loss_c4_path.append(loss_c4.item())
            self.loss_c5_path.append(loss_c5.item())
            self.c4_path.append(c4.item())
            self.c5_path.append(c5.item())
            
            index += 1
            
            if index % self.bayes_gap == 0:
                if negative_log_likelihood.item() >= loss0:
                    for g in optimizer.param_groups:
                        self.bayes_lr = self.bayes_lr * 0.5
                        g['lr'] = self.bayes_lr
                        
                loss0 = min(negative_log_likelihood.item(), loss0)
    ...
    def evaluation(self, sample_size = 1000):
        ...
        for t in tqdm(range(sample_size)):        
            ...
            loss_c5_list.append(loss_c5.item())
            c4_list.append(c4.item())
            c5_list.append(c5.item())
            
        self.y_pred_list = y_pred_list
        self.mse_list = mse_list
        self.c4_list = c4_list
        self.c5_list = c5_list
        self.loss_c4_list = loss_c4_list
        self.loss_c5_list = loss_c5_list
            
        return(np.average(self.mse_list), np.average(self.loss_c4_list), np.average(self.loss_c5_list), 
               np.average(self.c4_list), np.average(self.c5_list))
...
if __name__ == '__main__':
    ...
    SIGMA_1 = torch.FloatTensor([math.exp(-0)])
    SIGMA_2 = torch.FloatTensor([math.exp(-6)])
    model = SCBNN(train, test, neuron=50, bayes_step = 200, bayes_lr = 1e-3, bayes_gap = 20)
    model.sample_mode = "mean"
    model.bayes_fit()
    model.sample_mode = "sample"
    model.bayes_step = 200
    model.bayes_fit()
    print(model.evaluation())
    
    y_pred_mat = torch.cat(model.y_pred_list,axis=1).detach()
    y_pred_mat = np.array(y_pred_mat).T
    q_pred_mat = np.quantile(y_pred_mat, [0.2, 0.8], axis=0)
    ...
    plt.plot(model.negative_log_likelihood_path)
    plt.ylabel('negative_log_likelihood')
    plt.show()
    
    plt.plot(model.loss_c4_path)
    plt.ylabel('loss_c4')
    plt.show() 
    
    plt.plot(model.loss_c5_path)
    plt.ylabel('loss_c5')
    plt.show()
